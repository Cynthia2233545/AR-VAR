...
class CAR(VAR):
    ...
    def car_inference(
        ...
        if unet_features is not None:
            unet_features = self.unet_feature_processor(unet_features)
    ...
    def forward(self, label_B: torch.LongTensor, x_BLCv_wo_first_l: torch.Tensor,
        ...
        # 计算REPA损失（处理UNet特征维度）
        repa_loss = None
        if self.use_repa and intermediate_feature is not None and unet_features is not None:
            # 1. 处理UNet特征：从[B, 1, 256, 256]转换为[B, repa_dim]
